<template>
    <div class="p-16">
        <router-link :to="{name: ROUTES.HOME}" class="py-2 px-4 border-1 max-w-max rounded-sm shadow-sm dark:shadow-slate-900 flex gap-2 items-center transition ease-in-out transform hover:scale-110">
...
export default defineComponent({
    ...
    methods: {
        navigate(border: string) {
            const countryBorder = this.store.getCountry(border);
            if (countryBorder) {
                this.$router.push({name: ROUTES.COUNTRY, params: { id: countryBorder.name.common }});
            }
        },
        getCountry(id?: string) {
            ...
            if (country && typeof country === 'string') {
                this.store.fetchCountry(id || country);
            }
        }
    }
})
